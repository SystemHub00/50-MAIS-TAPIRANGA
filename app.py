...
# =============================================================================
LOCAL_OPTIONS = [
    {"id": "1", "nome": "IGREJA METODISTA EM PADRE MIGUEL"},
]
...
def fill_form_data_from_selection(form_data):
    ...
    if curso_id:
        matched = next(
            (o for o in COURSE_OPTIONS if o["curso_id"] == str(curso_id)), None)
        if matched:
            fill_form_data_from_option(form_data, matched)
            return
...
<head>
    ...
    <style>
        :root{--cor-principal:#0f4287;--cor-principal-escura:#0a2d5e;--cor-clara:#e8f0fb;--cor-texto:#0a2d5e;--cor-borda:#7faad4;--sombra-card:0 18px 55px rgba(15,66,135,0.18);}
        *{box-sizing:border-box;}html,body{min-height:100%;margin:0;padding:0;}
        body{min-height:100vh;background:radial-gradient(circle at top left,rgba(15,66,135,0.14),transparent 34%),radial-gradient(circle at top right,rgba(200,220,255,0.82),transparent 32%),linear-gradient(135deg,#f0f5ff 0%,#fff 42%,#d0e0f8 100%);color:var(--cor-texto);font-family:'Wise',Arial,sans-serif;}
        ...
        .wizard-progress{margin:18px auto 22px;padding:18px;border-radius:28px;background:rgba(255,255,255,0.9);box-shadow:0 12px 30px rgba(15,66,135,0.12);}
        .wizard-track{width:100%;height:14px;background:#c8d8f0;border-radius:999px;overflow:hidden;}
        .wizard-fill{height:100%;width:25%;background:linear-gradient(90deg,#0f4287 0%,#3a78c9 100%);border-radius:999px;transition:width 0.3s ease;}
        .wizard-labels{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:14px;}
        .wizard-label{padding:12px 10px;border:1px solid #a8c4e8;border-radius:18px;background:#fff;color:#1a4f8a;font-size:0.92rem;font-weight:700;text-align:center;transition:all 0.25s ease;}
        ...
        .hero-card{padding:32px;border-radius:30px;background:linear-gradient(135deg,#fff 0%,#f0f5ff 58%,#d0e0f8 100%);border:1px solid #a8c4e8;width:100%;text-align:center;}
        ...
        .hero-subtitle,.panel-subtitle{margin:0;color:#1a4f8a;font-size:1.05rem;line-height:1.55;}
        .hero-badge{display:inline-block;margin:8px 0 14px;padding:8px 20px;border-radius:999px;background:#fff;border:2px solid var(--cor-principal);color:var(--cor-principal);font-size:1rem;font-weight:800;letter-spacing:0.04em;}
        .hero-highlights{display:grid;gap:10px;margin-top:16px;}
        .hero-highlight,.info-card,.review-box,.step-card{border-radius:22px;border:1px solid #c0d4ee;background:#fff;box-shadow:0 8px 24px rgba(15,66,135,0.08);}
        .hero-highlight{padding:12px 14px;color:#1a4f8a;font-size:0.95rem;font-weight:700;}
        ...
        .benefits-nav{min-width:44px;min-height:44px;border:none;border-radius:999px;background:#fff;color:var(--cor-principal);box-shadow:0 6px 16px rgba(15,66,135,0.14);font:inherit;font-size:1.1rem;font-weight:900;cursor:pointer;}
        .benefits-dots{display:flex;gap:6px;align-items:center;justify-content:center;}
        .benefits-dot{width:9px;height:9px;border-radius:999px;background:#a8c4e8;transition:transform 0.2s ease,background 0.2s ease;}
        ...
        select:disabled{background-color:#dce8f5;cursor:not-allowed;opacity:0.7;}
        ...
        .form-group input,.form-group select,.form-group textarea{display:block;width:100%!important;max-width:100%!important;min-width:0!important;margin:0!important;box-sizing:border-box;min-height:38px;height:38px;padding:7px 10px;border:1.2px solid var(--cor-borda);border-radius:10px;background:#f0f5ff;color:var(--cor-texto);font:inherit;line-height:1.2;text-align:left;outline:none;transition:border-color 0.2s ease,box-shadow 0.2s ease,background 0.2s ease;}
        .form-group select{appearance:none;-webkit-appearance:none;-moz-appearance:none;background-image:url('data:image/svg+xml;utf8,<svg fill="%230f4287" height="20" viewBox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg"><path d="M7 10l5 5 5-5z"/></svg>');background-repeat:no-repeat;background-position:right 14px center;background-size:20px 20px;padding-right:44px;}
        .form-group textarea{min-height:60px;height:auto;resize:vertical;}
        .form-group input:focus,.form-group select:focus,.form-group textarea:focus{border-color:var(--cor-principal);background:#fff;box-shadow:0 0 0 4px rgba(15,66,135,0.12);}
        .readonly-field{background:#dce8f5!important;color:#0f4287!important;font-weight:700;}
        ...
        .icon-button{min-width:56px;min-height:52px;background:var(--cor-principal);color:#fff;box-shadow:0 8px 16px rgba(15,66,135,0.22);}
        ...
        .cta-button,.submit-button{background:linear-gradient(90deg,#0f4287 0%,#3a78c9 100%);color:#fff;box-shadow:0 10px 24px rgba(15,66,135,0.24);}
        ...
        .balao-erro{margin-top:4px;padding:10px 14px;border-radius:14px;border:1px solid #0a2d5e;background:#0f4287;color:#fff;font-size:0.92rem;font-weight:700;line-height:1.35;}
        .balao-erro[hidden]{display:none;}
        .erro-campo{border-color:#0f4287!important;box-shadow:0 0 0 4px rgba(15,66,135,0.12)!important;}
        ...
        .review-check{display:flex;gap:12px;align-items:flex-start;justify-content:flex-start;padding:10px 12px;border-radius:14px;background:var(--cor-clara);color:#0a2d5e;line-height:1.45;text-align:left;}
        ...
        @media(max-width:640px){
            ...
            .main-header{padding:8px 12px;}.header-logos{gap:12px;}.header-divider{height:36px;}.logo-prefeitura-topo,.logo-projeto-topo{height:36px;max-width:min(30vw,110px);}
...
<body data-start-step="{{ current_step }}">
    ...
    <header class="main-header">
        <div class="header-logos">
            ...
            <img src="/static/logoprojeto.jpeg"    alt="Emprega 50+ RJ"    class="logo-projeto-topo">
            <div class="header-divider"></div>
            <img src="/static/LogoEduThree.png"    alt="EduThree"          class="logo-projeto-topo">
        </div>
    </header>
            ...
            <form id="wizard-form" method="POST" action="{{ url_for('inscricao_unica') }}" autocomplete="off" novalidate>
                ...
                <section class="wizard-panel" data-step="index">
                    <div class="hero-grid"><div class="hero-card">
                        <span class="hero-pill">PROJETO: EMPREGA 50+ RJ</span>
                        <h1 class="hero-title">INCLUS&#195;O PRODUTIVA E VALORIZA&#199;&#195;O DA EXPERI&#202;NCIA</h1>
                        <div class="hero-badge">Combate ao Etarismo</div>
                        <p class="hero-subtitle">Qualifica&#231;&#227;o profissional gratuita para ampliar suas oportunidades e fortalecer sua entrada no mercado de trabalho.</p>
                        <div class="hero-highlights">
                            <div class="hero-highlight">
                                <strong>CURSOS DISPON&#205;VEIS:</strong>
                                <div class="cursos-lista">
                                    <span class="curso-tag">&#128194; Servi&#231;os Gerais</span>
                                    <span class="curso-tag">&#128194; Porteiro e Recep&#231;&#227;o</span>
                                    <span class="curso-tag">&#128194; Cuidador de Idosos</span>
                                    <span class="curso-tag">&#128194; Assistente Administrativo</span>
                                </div>
                            </div>
                            <div class="hero-highlight">
                            ...
                            <div class="hero-highlight">
                                <strong>EMPREGA 50+ RJ</strong>
                                Programa de inclus&#227;o produtiva com foco na valoriza&#231;&#227;o da experi&#234;ncia de vida e no combate ao etarismo no mercado de trabalho.
                            </div>
                        </div>
                ...
                <section class="wizard-panel" data-step="revisao">
                    <div class="step-card"><h2 class="panel-title">Revise antes de finalizar</h2>
                        ...
                        <div class="review-layout">
                            ...
                            <div class="review-box full">
                                <div style="margin-bottom:10px;color:#1a4f8a;font-size:0.98rem;text-align:left;"><strong>Elegibilidade:</strong> Este curso &#233; destinado a pessoas com 50 anos ou mais interessadas em qualifica&#231;&#227;o profissional.</div>
...
<head>
    ...
    <script>
        ...
        fbq('init','2008632536670997');fbq('track','PageView');
    </script>
    <style>
        :root{--cor-principal:#0f4287;--cor-clara:#e8f0fb;--cor-texto:#0a2d5e;--sombra-card:0 18px 55px rgba(15,66,135,0.18);}
        body{min-height:100vh;margin:0;background:radial-gradient(circle at top left,rgba(15,66,135,0.15),transparent 32%),linear-gradient(140deg,#f0f5ff 0%,#fff 55%,#d0e0f8 100%);font-family:'Wise',Arial,sans-serif;}
        .main-header{border-bottom:4px solid #0f4287;background:rgba(255,255,255,0.92);padding:10px 20px;}
        .header-logos{display:flex;align-items:center;justify-content:center;gap:24px;flex-wrap:wrap;}
        .header-divider{width:1px;height:52px;background:linear-gradient(to bottom,transparent,#7faad4,transparent);flex-shrink:0;}
        .logo-prefeitura-topo,.logo-projeto-topo{height:52px;width:auto;object-fit:contain;}
        .confirm-page{width:min(680px,calc(100% - 16px));margin:0 auto;padding:10px 0 20px;text-align:center;}
        .wizard-progress{margin:12px auto 16px;padding:14px;border-radius:28px;background:rgba(255,255,255,0.9);box-shadow:0 12px 30px rgba(15,66,135,0.12);}
        .wizard-track{width:100%;height:14px;border-radius:999px;background:#c8d8f0;overflow:hidden;}
        .wizard-fill{width:100%;height:100%;background:linear-gradient(90deg,#0f4287 0%,#3a78c9 100%);border-radius:999px;}
        .wizard-labels{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:12px;}
        .wizard-label{padding:10px 8px;border:1px solid #a8c4e8;border-radius:16px;background:#fff;color:#1a4f8a;font-size:0.84rem;font-weight:700;text-align:center;}
        .wizard-label.ativo{border-color:#0f4287;background:#e8f0fb;color:#0f4287;}
        ...
        .checkmark svg{width:100%;height:100%;stroke:#0f4287;fill:none;}
        .confirm-card h1{margin:0 0 10px;color:#0f4287;font-size:clamp(1.8rem,4vw,2.6rem);letter-spacing:-0.04em;}
        .protocol-box{margin:16px auto 12px;padding:14px;max-width:320px;border-radius:16px;background:#e8f0fb;border:2px solid #0f4287;}
        .protocol-box strong{display:block;color:#0f4287;font-size:0.98rem;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.04em;}
        .protocol-box span{display:block;color:#0f4287;font-size:1.35rem;font-weight:900;letter-spacing:0.08em;word-break:break-all;}
        .next-steps{margin:16px auto 0;max-width:460px;padding:14px;border-radius:18px;background:#fff;border:1px solid #c0d4ee;}
        .next-steps h2{margin:0 0 12px;color:#0f4287;font-size:1.2rem;}
        .next-steps ol{margin:0;padding-left:22px;color:#1a4f8a;line-height:1.55;list-style-position:inside;}
        ...
        .action-button.primary{background:linear-gradient(90deg,#0f4287 0%,#3a78c9 100%);color:#fff;box-shadow:0 10px 24px rgba(15,66,135,0.24);}
        .action-button.secondary{background:#fff;color:#0f4287;border:2px solid #0f4287;}
        .action-button:hover{transform:translateY(-1px);}
        @media(max-width:640px){html,body{width:100%!important;max-width:100%!important;overflow-x:hidden!important;}body*{min-width:0;}.main-header{padding:8px 12px;}.header-logos{gap:12px;}.header-divider{height:36px;}.logo-prefeitura-topo,.logo-projeto-topo{height:36px;max-width:min(30vw,110px);}.confirm-page{width:calc(100% - 8px)!important;max-width:100%!important;padding:6px 0 12px;}.confirm-card{width:100%!important;max-width:100%!important;padding:14px 10px 12px;}.wizard-progress{width:100%!important;max-width:100%!important;padding:10px;border-radius:18px;}.wizard-labels{grid-template-columns:1fr;gap:6px;}.confirm-shell{width:100%!important;max-width:100%!important;border-radius:18px;}.next-steps,.actions,.action-button,.protocol-box,.wizard-label,.wizard-track{width:100%!important;max-width:100%!important;}img,svg{max-width:100%!important;height:auto!important;}}
    </style>
</head>
<body>
    <noscript><img height="1" width="1" style="display:none" src="https://www.facebook.com/tr?id=2008632536670997&ev=PageView&noscript=1"/></noscript>
...
def confirmacao():
    ...
    home_url="https://five0-mais-rj.onrender.com"
    return render_template_string(
        TEMPLATE_CONFIRMACAO,
        protocolo=protocolo,
